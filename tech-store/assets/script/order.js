...
function createHistoricOrder(orderWithDate) {
    const orderElement = `<section id="section-order-${orderWithDate.date}" class="section-order">
    <p>${ new Date(orderWithDate.date).toLocaleDateString("pt-br", {hour: "2-digit", minute: "2-digit"})}</p>
    </section>`;
